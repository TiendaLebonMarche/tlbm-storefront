...
export default function CollectionsDropdown({
  ...
  itemClassName = "group flex items-center justify-between gap-2 px-3 py-2.5 rounded-lg text-[12px] font-semibold uppercase tracking-[0.14em] text-black/60 hover:text-gold-deep hover:bg-gold/6 transition-all duration-200",
  ...
  return (
    ...
    >
      ...
      <div
        ...
        `}
      >
        <div className={`${panelWidth} bg-white border border-gold/15 rounded-2xl shadow-[0_24px_60px_rgba(10,10,15,0.14)] p-5`}>
          {/* Header del panel */}
          ...
          <div className="grid grid-cols-2 gap-1">
            {collections.length === 0 ? (
              ...
              collections.map((collection) => (
                ...
                >
                  {collection.title}
                  <span className="text-gold/50 text-[10px] opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200">
                    →
                  </span>
                </LocalizedClientLink>
              ))
          ...
          <LocalizedClientLink
            href="/collections"
            className="mt-4 pt-4 border-t border-white flex items-center justify-between text-[10px] font-bold uppercase tracking-[0.28em] text-black/50 hover:text-gold-deep transition-colors duration-200"
          >
            Ver todas las colecciones
