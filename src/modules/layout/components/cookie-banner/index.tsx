    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className="fixed bottom-0 left-0 right-0 z-[100] px-3 pb-3 md:px-4 md:pb-4 pointer-events-none flex justify-center"
        >
          <div className="w-full max-w-3xl bg-brand-black/90 backdrop-blur-xl shadow-2xl rounded-sm p-4 md:p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 pointer-events-auto border border-white/10">
            ...
            <div className="flex-shrink-0 flex gap-2 w-full md:w-auto">
              <button 
                className="w-full md:w-auto bg-transparent border border-white/20 text-white/70 hover:text-white hover:border-white/40 text-[10px] font-semibold uppercase tracking-wider px-4 py-2 transition-all duration-300"
                onClick={() => window.location.href = '/legal/privacidad'}
              >
                Privacidad
              </button>
              <button 
                className="w-full md:w-auto bg-white text-brand-black text-[10px] font-bold uppercase tracking-wider px-5 py-2 hover:bg-gray-100 transition-all duration-300"
