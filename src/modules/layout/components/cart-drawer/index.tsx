...
// Tiempo en ms antes de cerrar la bolsa automáticamente (home: al cambiar items)
...
/**
 * DRAWER DE LA BOLSA — instancia ÚNICA global (se monta desde el layout vía
 * cart-drawer-data). Fix 10-ago: antes cada CartButton montaba su propio
 * drawer con createPortal → 2 paneles duplicados en mobile (el del bloque
 * desktop escapaba del hidden) + Popover/useId (React #441 server-side).
 * Ahora: divs + Transition (patrón SideMenu) — sin Popover, sin portal.
 */
const CartDrawer = ({ cart: cartProp }: { cart?: HttpTypes.StoreCart | null }) => {
  const [activeTimer, setActiveTimer] = useState<ReturnType<typeof setTimeout> | undefined>(
    undefined
  )
  const { isCartOpen, openCart, closeCart, isSideMenuOpen, closeSideMenu, cart: contextCart } = useUI()
  ...
  // 1) Ocultar el header fijo (marquesina oscura).
  // 2) Bloquear el scroll del fondo (el drawer scrollea internamente).
  // Instancia ÚNICA → sin necesidad de estado compartido entre instancias.
  ...
  return (
    <>
      {/* Overlay oscuro (clic fuera para cerrar — franja izquierda visible en móvil) */}
      <Transition
        show={isCartOpen}
        as={Fragment}
          ...
          {cartState && cartState.items?.length ? (
            <>
              ...
              <div className="px-5 py-4 sm:px-6 sm:py-5 border-t border-brand-gray-light bg-white">
                ...
                {/* Botón Ver Bolsa (link directo — sin button anidado para evitar a11y duplicada) */}
                <LocalizedClientLink
                  href="/cart"
                  onClick={close}
                  className="w-full bg-brand-black text-white py-3.5 sm:py-4 text-xs font-bold uppercase tracking-[0.2em] hover:bg-gray-900 transition-colors rounded-lg mb-3 flex items-center justify-center"
                >
                  Ver Bolsa y Pagar
                </LocalizedClientLink>
