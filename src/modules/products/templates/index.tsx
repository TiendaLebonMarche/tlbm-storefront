...
const ProductTemplate: React.FC<ProductTemplateProps> = ({
  ...
  return (
    <div className="bg-white min-h-screen selection:bg-brand-brown/10">

      {/* ── BREADCRUMB ── */}
      <div className="border-b border-gray-50">
        <div className="content-container py-4 md:py-5">
          <nav className="flex items-center gap-2 text-[9px] text-gray-400 tracking-[0.2em] uppercase font-sans">
            ...
            {product.collection && (
              <>
                <span className="text-gray-200">/</span>
                <span className="text-brand-brown/60 font-medium">{product.collection.title}</span>
              </>
            )}
          </nav>
        </div>
          ...
          <div className="w-full lg:w-[42%] lg:mt-4">
            ...
            <div className="space-y-6 pb-8 border-b border-gray-100/60">
              ...
              <div className="flex items-center gap-4 py-1">
                ...
                <span className="text-[11px] text-gray-400 font-sans uppercase tracking-[0.15em] border-l border-gray-200 pl-4">
                ...
                {totalStock > 0 ? (
                  <>
                    <div className="relative flex items-center justify-center">
                      <div className="w-2 h-2 rounded-full bg-brand-olive/40 animate-ping absolute" />
                      <div className="w-2 h-2 rounded-full bg-brand-olive relative" />
                    </div>
                    <div>
                      <p className="text-sm font-medium text-brand-brown font-sans">Disponible</p>
                      <p className="text-[11px] text-gray-400 font-light font-sans tracking-tight">Envío prioritario desde nuestro atelier en Bucaramanga</p>
                    </div>
                  </>
                ) : (
                  <>
            ...
            <div className="py-8 grid grid-cols-2 gap-x-8 gap-y-6">
              {[
                ...
                <div key={i} className="flex items-start gap-4 group/trust">
                  <div className="flex-shrink-0 text-brand-brown/30 group-hover/trust:text-brand-brown transition-colors duration-500 mt-0.5">
                    {item.icon}
                  </div>
                  <div className="space-y-0.5">
                    <p className="text-[10px] font-bold text-brand-brown font-sans uppercase tracking-[0.1em]">{item.label}</p>
                    <p className="text-[10px] text-gray-400 font-light font-sans tracking-tight">{item.sub}</p>
                  </div>
                </div>
              ))}
            </div>
