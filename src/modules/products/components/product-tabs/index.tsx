...
const ProductTabs = ({ product }: ProductTabsProps) => {
      ...
      component: (
        <div className="py-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
          {product.description ? (
            ...
            <p className="text-sm text-brand-gray font-light italic">No hay descripción disponible para este producto.</p>
          )}
        </div>
      ),
    },
  ...
  return (
    <div className="w-full">
      {/* Tab Headers */}
      <div className="flex items-center gap-10 border-b border-brand-gray-light overflow-x-auto no-scrollbar">
        {tabs.map((tab, i) => (
          <button
            key={i}
          ...
          >
            {tab.label}
            {activeTab === i && (
              <div className="absolute bottom-0 left-0 w-full h-0.5 bg-brand-black animate-in fade-in zoom-in-x duration-500" />
            )}
          </button>
        ))}
      </div>
  ...
  return (
    <div className="py-4 space-y-6">
      ...
      {specs.length > 0 ? (
        ...
        <p className="text-sm text-brand-gray font-light italic">Especificaciones no disponibles.</p>
      )}
      ...
      {product.variants && product.variants.length > 1 && (
        <div className="pt-2">
          <p className="text-[10px] text-brand-gray uppercase tracking-[0.2em] mb-3 font-sans">Variantes disponibles</p>
          <div className="flex flex-wrap gap-2">
