...
const Hero = () => {
  ...
  const kawsTranslateY = useTransform(scrollY, [0, 1000], [0, 250])
  const kawsScale = useTransform(scrollY, [0, 800], [1, 0.85])
  const kawsRotate = useTransform(scrollY, [0, 800], [0, -5])
  ...
  return (
    <section 
      ref={containerRef}
      className="relative w-full h-[110vh] min-h-[850px] flex flex-col items-center justify-between overflow-hidden bg-[#F2F2E1]"
    >
      {/* Capa de textura sutil de papel premium */}
      <div className="absolute inset-0 opacity-[0.06] pointer-events-none mix-blend-multiply z-0">
      ...
      <div className="relative z-30 w-full px-6 md:px-12 lg:px-20 pt-32 md:pt-48 grid grid-cols-1 lg:grid-cols-2 gap-y-12 pointer-events-none select-none">
        ...
        >
          <div className="max-w-[320px]">
            <p className="font-serif italic text-brand-brown text-xl md:text-2xl leading-[1.15] tracking-tight reveal-up">
        ...
        >
          ...
          {/* Descripción Juvenil */}
          <div className="mt-10 lg:mt-14 max-w-[420px] reveal-right delay-200">
            <p className="font-sans font-bold text-[13px] md:text-sm text-brand-brown/70 uppercase tracking-wide leading-[1.6]">
      ...
      {/* 2. ELEMENTO CENTRAL: BLACK COMPANION KAWS (SHARE VERSION) */}
      <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none pt-32 lg:pt-20">
        <motion.div
           style={{
             x: xSpring,
             y: ySpring,
           ...
           className="relative h-[85%] w-full max-w-5xl flex items-center justify-center"
        >
          <div className="relative w-full h-full flex items-center justify-center">
            {/* Imagen del KAWS Companion SHARE proporcionada por el usuario */}
            <Image
              src="https://res.cloudinary.com/dgo9tm9e2/image/upload/v1775591353/https___hypebeast.com_image_2020_02_kaws-share-companion-vinyl-figures-release-info-8-artguru_iuifpu.png" 
              alt="Black Companion KAWS Share"
              width={1200}
              height={1200}
              className="object-contain drop-shadow-[0_50px_100px_rgba(0,0,0,0.2)]"
              priority
            />
          </div>
        </motion.div>
      ...
      <div className="relative z-30 w-full px-6 md:px-12 lg:px-20 pb-12 flex flex-col md:flex-row justify-between items-end gap-8 text-brand-brown/40 select-none">
        <div className="flex flex-col">
          <span className="text-[11px] uppercase tracking-[0.4em] font-black opacity-30 mb-1">Title —</span>
          <span className="font-sans font-bold text-xs md:text-sm">ANTIGRAVITY ECOMMERCE</span>
        </div>
        <div className="flex flex-col">
          <span className="text-[11px] uppercase tracking-[0.4em] font-black opacity-30 mb-1">Date —</span>
          <span className="font-sans font-bold text-xs md:text-sm">07.04.2026</span>
        </div>
        <div className="flex flex-col">
          <span className="text-[11px] uppercase tracking-[0.4em] font-black opacity-30 mb-1">Time —</span>
          <span className="font-sans font-bold text-xs md:text-sm">11:30 GMT-5</span>
        </div>
        <div className="flex flex-col">
          <span className="text-[11px] uppercase tracking-[0.4em] font-black opacity-30 mb-1">Theme —</span>
